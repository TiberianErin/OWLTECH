    ...
    try:
        while True:
            ...
            if pressure > 90.0 and not any(e[0] == "P001" for e in active_errors):
                new_error = ("P001", "[ERR] Critical Pressure Overload", {"temperature": +3.0, "pressure": -3.0, "fix_message": "ADJUSTING PRESSURE VALVES"})
                active_errors.append(new_error)
                beep(700, 100)
            if temperature > 900.0 and not any(e[0] == "T001" for e in active_errors):
                new_error = ("T001", "[ERR] Temperature Exceeding Safe Limits", {"temperature": -3.0, "pressure": +3.0, "fix_message": "COOLING CORE"})
                active_errors.append(new_error)
                beep(700, 100)
            if airflow < 15.0 and not any(e[0] == "A001" for e in active_errors):
                new_error = ("A001", "[ERR] Ventilation Ducts Blockage", {"temperature": +3.0, "airflow": +3.0, "fix_message": "RESTARTING VENTILATION"})
                active_errors.append(new_error)
                beep(700, 100)
            ...
            if maintenance_mode and fixing_error:
                ...
                if fix_progress >= 1.0:
                    ...
                    fix_start_time = None
                    maintenance_mode = False
                    temperature -= 5.0  # Successful fix reduces temperature
                    pressure -= 5.0  # Successful fix reduces pressure
                    airflow += 10.0  # Successful fix increases airflow
                    total_duration += 30
                    beep(1000, 300)
            else:
